...
configuration — it is fifteen standalone projects that other people copy:
eleven flakes, and four devenv environments. That one fact drives every rule below. If a
...
4. **A template declares its `kind`, and the kind decides its shape.**
   `kind = "flake"` — eleven of the fifteen, and the default — means **one input,
...
git repo — eleven flakes and four devenv environments. They share no code and cannot.
