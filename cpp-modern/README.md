...
Dev environment for modern C++ with modules support — C++23 with full `import`
support, on LLVM 21.

```bash
nix flake init -t github:Marcus441/nix-templates#cpp-modern
...
Linux. Module dependency scanning uses `clang-scan-deps` under Clang and the
compiler itself under GCC (>= 14). `CC`/`CXX` are set by the shell, so the same
CMake presets work in both:
...
- This template and `cpp` share most of their scaffold. Templates are copied
  verbatim and cannot import shared code, so the duplication is deliberate and
  permanent — a fix to one is worth applying to both.
