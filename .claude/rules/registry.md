...
| `welcomeText` | Almost always leave unset. `standardWelcome` in `registry.nix` builds the post-init message from `description`, and one message across thirteen templates is the point. Set it only to say something that text cannot. |
